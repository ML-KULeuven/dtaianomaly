...
@pytest.mark.parametrize(
    "plot_function,additional_args,obligated_y_pred",
    [
...
class TestPlottingFunctions:
    ...
    def test_univariate(
        self, plot_function, additional_args, obligated_y_pred, univariate_time_series
    ):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        if obligated_y_pred:
            additional_args["y_pred"] = np.random.choice(
                [0, 1], size=univariate_time_series.shape[0], replace=True
            )
        plot_function(univariate_time_series, y=y, **additional_args)

    def test_multivariate(
        self, plot_function, additional_args, obligated_y_pred, multivariate_time_series
    ):
        y = np.random.choice(
            [0, 1], size=multivariate_time_series.shape[0], replace=True
        )
        if obligated_y_pred:
            additional_args["y_pred"] = np.random.choice(
                [0, 1], size=multivariate_time_series.shape[0], replace=True
    ...
    def test_give_time_steps(
        self, plot_function, additional_args, obligated_y_pred, univariate_time_series
    ):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        time_steps = (
            visualization.format_time_steps(None, univariate_time_series.shape[0]) * 2
            + 10
        )
        if obligated_y_pred:
            additional_args["y_pred"] = np.random.choice(
        ...
        plot_function(
            univariate_time_series, y=y, time_steps=time_steps, **additional_args
        )

    def test_given_feature_names_univariate(
        self, plot_function, additional_args, obligated_y_pred, univariate_time_series
    ):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        if obligated_y_pred:
        ...
        plot_function(
            univariate_time_series, y=y, feature_names=feature_names, **additional_args
        )

    def test_given_feature_names_multivariate(
        self, plot_function, additional_args, obligated_y_pred, multivariate_time_series
    ):
        y = np.random.choice(
            [0, 1], size=multivariate_time_series.shape[0], replace=True
        ...
        feature_names = [
            f"dimension {i}" for i in range(multivariate_time_series.shape[1])
        ]
        plot_function(
    ...
    def test_given_feature_names_different_dimensionality(
        self, plot_function, additional_args, obligated_y_pred, univariate_time_series
    ):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        if obligated_y_pred:
...
class TestNonBinaryPrediction:
    ...
    def test_non_binary_y_pred(
        ...
        if obligated_y_pred:
            ...
            additional_args["y_pred"] = np.random.uniform(
                size=univariate_time_series.shape[0]
            )
            with pytest.raises(ValueError):
                plot_function(univariate_time_series, y, **additional_args)
...
@pytest.mark.parametrize(
    "plot_function,additional_args,obligated_y,obligated_y_pred",
    [
        ...
        (
            lambda X, ax, time_steps=None: None,
            {},
            False,
            False,
...
class TestPlotWithZoom:
    ...
    def test(
        ...
        if obligated_y:
            additional_args["y"] = np.random.choice(
                [0, 1], size=univariate_time_series.shape[0], replace=True
            )
        if obligated_y_pred:
            additional_args["y_pred"] = np.random.choice(
        ...
        visualization.plot_with_zoom(
            univariate_time_series,
            start_zoom=100,
            end_zoom=200,
            method_to_plot=plot_function,
            **additional_args,
        )

    def test_given_time_steps(
        self,
        plot_function,
        additional_args,
        obligated_y,
...
class TestPlotAnomalyScoresMultiplePredictions:
    ...
    def test_univariate(self, univariate_time_series):
        y = np.random.choice([0, 1], size=univariate_time_series.shape[0], replace=True)
        y_pred = {
            f"model-{i}": np.random.uniform(size=univariate_time_series.shape[0])
            for i in range(5)
        }
    ...
    def test_multivariate(self, multivariate_time_series):
        ...
        y_pred = {
            f"model-{i}": np.random.uniform(size=multivariate_time_series.shape[0])
            for i in range(5)
        }
    ...
    def test_confidence_given(self, univariate_time_series):
        ...
        with pytest.raises(ValueError):
            visualization.plot_anomaly_scores(
                univariate_time_series, y, y_pred, confidence=confidence
            )
...
class TestPlotConfidence:
    ...
    def test_multivariate(self, multivariate_time_series):
        ...
        visualization.plot_anomaly_scores(
            multivariate_time_series, y, y_pred, confidence=confidence
        )

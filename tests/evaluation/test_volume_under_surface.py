...
from dtaianomaly.evaluation.volume_under_surface import (
    RangeAreaUnderPR,
    RangeAreaUnderROC,
    VolumeUnderPR,
    VolumeUnderROC,
...
class TestInitialization:
    ...
    @pytest.mark.parametrize(
        "metric", [RangeAreaUnderROC, RangeAreaUnderPR, VolumeUnderROC, VolumeUnderPR]
    )
...
class TestStr:
    ...
    def test_range_roc(self):
        ...
        assert (
            str(RangeAreaUnderPR(buffer_size=50)) == "RangeAreaUnderPR(buffer_size=50)"
        )
        assert (
            str(RangeAreaUnderPR(compatibility_mode=True))
            == "RangeAreaUnderPR(compatibility_mode=True)"
        )
        assert (
            str(RangeAreaUnderPR(buffer_size=50, max_samples=150))
            == "RangeAreaUnderPR(buffer_size=50,max_samples=150)"
        )

    def test_range_pr(self):
        assert str(RangeAreaUnderROC()) == "RangeAreaUnderROC()"
        assert (
            str(RangeAreaUnderROC(buffer_size=50))
            == "RangeAreaUnderROC(buffer_size=50)"
        )
        assert (
            str(RangeAreaUnderROC(compatibility_mode=True))
            == "RangeAreaUnderROC(compatibility_mode=True)"
        )
        assert (
            str(RangeAreaUnderROC(buffer_size=50, max_samples=150))
            == "RangeAreaUnderROC(buffer_size=50,max_samples=150)"
        )

    def test_vus_roc(self):
        assert str(VolumeUnderPR()) == "VolumeUnderPR()"
        assert (
            str(VolumeUnderPR(max_buffer_size=50))
            == "VolumeUnderPR(max_buffer_size=50)"
        )
        assert (
            str(VolumeUnderPR(compatibility_mode=True))
            == "VolumeUnderPR(compatibility_mode=True)"
        )
        assert (
            str(VolumeUnderPR(max_buffer_size=50, max_samples=150))
            == "VolumeUnderPR(max_buffer_size=50,max_samples=150)"
        )

    def test_vus_pr(self):
        assert str(VolumeUnderROC()) == "VolumeUnderROC()"
        assert (
            str(VolumeUnderROC(max_buffer_size=50))
            == "VolumeUnderROC(max_buffer_size=50)"
        )
        assert (
            str(VolumeUnderROC(compatibility_mode=True))
            == "VolumeUnderROC(compatibility_mode=True)"
        )
        assert (
            str(VolumeUnderROC(max_buffer_size=50, max_samples=150))
            == "VolumeUnderROC(max_buffer_size=50,max_samples=150)"
        )

    @pytest.mark.parametrize(
        "metric,string",
        [(VolumeUnderROC(), "VolumeUnderROC()"), (VolumeUnderPR(), "VolumeUnderPR()")],
    )

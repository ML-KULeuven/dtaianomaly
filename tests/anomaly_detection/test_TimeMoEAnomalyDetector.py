...
from dtaianomaly.anomaly_detection import TimeMoEAnomalyDetector, Supervision
...
class TestTimeMoEAnomalyDetector:
    ...
    def test_str(self):
        ...
        assert str(TimeMoEAnomalyDetector('fft')) == "TimeMoEAnomalyDetector(window_size='fft')"
        assert str(TimeMoEAnomalyDetector(15, 'TimeMoE-200M')) == "TimeMoEAnomalyDetector(window_size=15,model_path='TimeMoE-200M')"
        assert str(TimeMoEAnomalyDetector(25, batch_size=3)) == "TimeMoEAnomalyDetector(window_size=25,batch_size=3)"
    ...
    def test_prediction_length_valid(self, prediction_length):
        detector = TimeMoEAnomalyDetector(window_size=16, prediction_length=prediction_length)
    ...
    def test_do_fine_tuning_valid(self, normalize_sequences):
        detector = TimeMoEAnomalyDetector(window_size='fft', normalize_sequences=normalize_sequences)
    ...
    def test_do_fine_tuning_invalid_type(self, normalize_sequences):
        with pytest.raises(TypeError):
            TimeMoEAnomalyDetector(window_size='fft', normalize_sequences=normalize_sequences)
...
class TestGetBatchStarts:
    ...
    def test_simple(self):
        time_moe = TimeMoEAnomalyDetector(window_size=2, batch_size=4, prediction_length=1)
    ...
    @pytest.mark.parametrize('batch_size,window_size,length_time_series,prediction_length', [
        (4, 2, 11, 1),
        (16, 4, 100, 1),
        (16, 4, 100, 5),
    ...
    def test(self, batch_size, window_size, length_time_series, prediction_length):
        time_moe = TimeMoEAnomalyDetector(window_size=window_size, batch_size=batch_size, prediction_length=prediction_length)
        ...
        for i, batch in enumerate(starts):
            ...
            assert np.array_equal(batch, np.arange(batch_size*i, batch_size*i + len(batch)))
            total_nb_samples += len(batch)

        assert total_nb_samples == length_time_series - window_size - prediction_length + 1

...
class TestChronosAnomalyDetector:
    ...
    def test_str(self, monkeypatch):
        ...
        assert (
            str(MOMENTAnomalyDetector("fft"))
            == "MOMENTAnomalyDetector(window_size='fft')"
        )
        assert (
            str(MOMENTAnomalyDetector(15, "large"))
            == "MOMENTAnomalyDetector(window_size=15,model_size='large')"
        )
        assert (
            str(MOMENTAnomalyDetector(25, batch_size=3))
            == "MOMENTAnomalyDetector(window_size=25,batch_size=3)"
        )
        cleanup()
    ...
    def test_do_fine_tuning_valid(self, do_fine_tuning, monkeypatch):
        setup(monkeypatch)
        detector = MOMENTAnomalyDetector(

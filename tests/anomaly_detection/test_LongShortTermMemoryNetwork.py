...
class TestLongShortTermMemoryNetwork:
    ...
    def test_supervision(self):
        assert LongShortTermMemoryNetwork(window_size=100).supervision == Supervision.SEMI_SUPERVISED

    def test_str(self):
        assert str(LongShortTermMemoryNetwork(window_size=100)) == "LongShortTermMemoryNetwork(window_size=100)"
        assert str(LongShortTermMemoryNetwork(window_size=100, hidden_units=17)) == "LongShortTermMemoryNetwork(window_size=100,hidden_units=17)"
        assert str(LongShortTermMemoryNetwork(window_size=100, dropout_rate=0.5)) == "LongShortTermMemoryNetwork(window_size=100,dropout_rate=0.5)"
...
class TestInitialize:
    ...
    def test_num_lstm_layers_valid(self, num_lstm_layers):
        detector = LongShortTermMemoryNetwork(window_size=16, num_lstm_layers=num_lstm_layers)
...
class TestBuildArchitecture:
    ...
    def test_custom(self, forecast_length, hidden_units, num_lstm_layers, bias, dropout_rate):
        ...
        assert is_flatten(next(modules))
        assert is_linear(next(modules), 16 * forecast_length * hidden_units, 8 * forecast_length)

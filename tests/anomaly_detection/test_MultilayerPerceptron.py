...
class TestMultilayerPerceptron:
    ...
    def test_supervision(self):
        assert MultilayerPerceptron(window_size=100).supervision == Supervision.SEMI_SUPERVISED

    def test_str(self):
        assert str(MultilayerPerceptron(window_size=100)) == "MultilayerPerceptron(window_size=100)"
        assert str(MultilayerPerceptron(window_size=100, hidden_layers=[64, 32, 16])) == "MultilayerPerceptron(window_size=100,hidden_layers=[64, 32, 16])"
        assert str(MultilayerPerceptron(window_size=100, dropout_rate=0.5)) == "MultilayerPerceptron(window_size=100,dropout_rate=0.5)"
...
class TestInitialize:
    ...
    def test_activation_function_valid(self, activation_function):
        detector = MultilayerPerceptron(window_size=16, activation_function=activation_function)
    ...
    def test_activation_functions_invalid_type(self, activation_function):
        with pytest.raises(TypeError):
            MultilayerPerceptron(window_size=16, activation_function=activation_function)
    ...
    def test_batch_normalization_valid(self, normalization):
        detector = MultilayerPerceptron(window_size=16, encoder_batch_normalization=normalization)
    ...
    def test_batch_normalization_valid(self, batch_normalization):
        detector = MultilayerPerceptron(window_size='fft', batch_normalization=batch_normalization)

...
class TestNoneAttribute:
    ...
    def test_get_valid_value_description(self):
        sorted_window_sizes = ', '.join(f'\'{v}\'' for v in sorted(AUTO_WINDOW_SIZE_COMPUTATION))
        assert WindowSizeAttribute()._get_valid_value_description() == f"greater than or equal to 1 or in {{{sorted_window_sizes}}}"

...
from dtaianomaly.thresholding import (
    ContaminationRateThreshold,
    FixedCutoffThreshold,
    TopNThreshold,
)
from dtaianomaly.workflow import JobBasedWorkflow, Workflow
from dtaianomaly.workflow._JobBasedWorkflow import (
    _convert_to_proba_metrics,
...
class TestWorkflowSuccess:
    ...
    def test(self, tmp_path_factory, show_progress):
        ...
        assert (
            results["Detector"].value_counts()["LocalOutlierFactor(window_size=15)"]
            == 2
        )
        assert (
            results["Detector"].value_counts()["IsolationForest(window_size=15)"] == 2
        )
    ...
    def test_with_kwargs(self, tmp_path_factory, show_progress):
        ...
        assert results["Preprocessor"].value_counts()["StandardScaler()"] == 2
        assert (
            results["Detector"].value_counts()["LocalOutlierFactor(window_size='fft')"]
            == 2
        )
        assert (
            results["Detector"].value_counts()["IsolationForest(window_size='fft')"]
            == 2
        )
        assert "Peak Memory Fit [MB]" not in results.columns
        assert "Peak Memory Predict [MB]" not in results.columns
    ...
    def test_no_preprocessors(
        self, tmp_path_factory, univariate_time_series, show_progress
    ):
        workflow = Workflow(
            dataloaders=[DemonstrationTimeSeriesLoader()],
            metrics=[Precision(), Recall(), AreaUnderROC()],
        ...
        assert (
            results["Detector"].value_counts()["IsolationForest(window_size=15)"] == 1
        )
        assert "Peak Memory Fit [MB]" in results.columns
        assert "Peak Memory Predict [MB]" in results.columns
        assert "Peak Memory [MB]" in results.columns
...
class TestWorkflowFail:
    ...
    def test_not_compatible(self, tmp_path_factory):
        ...
        expected_error_message = (
            f"Not compatible: detector with supervision Supervision.SUPERVISED "
...
class TestGetTrainTestData:
    ...
    def test_unsupervised(
        self, fit_unsupervised_on_test_data, fit_semi_supervised_on_test_data
    ):
        data_set = DataSet(
...
class TestShowProgress:
    ...
    def test_show_progress_tqdm_not_installed(
        self, monkeypatch, tmp_path_factory, n_jobs
    ):
    ...
    def test_do_not_show_progress_tqdm_installed(
        self, monkeypatch, tmp_path_factory, n_jobs
    ):
    ...
    def test_do_not_show_progress_tqdm_not_installed(
        self, monkeypatch, tmp_path_factory, n_jobs
    ):
        mock = MagicMock(side_effect=Exception)
        monkeypatch.setitem(sys.modules, "tqdm", MagicMock(tqdm=mock))

...
class TestCheckIsValid:
    ...
    def test_is_valid_given_train_data(self, valid_X_test, valid_y_test, valid_X_train):
        data_set = DataSet(
            X_test=valid_X_test, y_test=valid_y_test, X_train=valid_X_train
        )
        assert data_set.is_valid()

    def test_is_valid_given_train_data_and_labels(
        self, valid_X_test, valid_y_test, valid_X_train, valid_y_train
    ):
        data_set = DataSet(
            X_test=valid_X_test,
            y_test=valid_y_test,
    ...
    def test_not_is_valid(self, valid_X_test, valid_y_test):
        ...
        data_set.X_test = [
            ...
            "4",
            5,
        ]  # Make data invalid (should not be done!)
    ...
    def test_multivariate_y_test(self, valid_X_test):
        with pytest.raises(ValueError):
            DataSet(
                X_test=valid_X_test,
                y_test=np.array([[0, 0], [0, 1], [1, 1], [0, 0], [1, 1], [0, 0]]),
            )
    ...
    def test_X_test_and_X_train_different_dimension(
        self, valid_X_test, valid_y_test, valid_X_train
    ):
        multivariate_valid_X_train = np.repeat(valid_X_train, 2).reshape(
            valid_X_train.shape[0], -1
        )
        with pytest.raises(ValueError):
            DataSet(
                X_test=valid_X_test,
                y_test=valid_y_test,
                X_train=multivariate_valid_X_train,
            )

    def test_valid_y_train_but_no_X_train(
        self, valid_X_test, valid_y_test, valid_y_train
    ):
        with pytest.raises(ValueError):
    ...
    def test_invalid_y_train(self, valid_X_test, valid_y_test, valid_X_train):
        with pytest.raises(ValueError):
            DataSet(
                ...
                y_test=valid_y_test,
                X_train=valid_X_train,
                y_train=np.array([0, 0, 1, 0, "1", 0, 0]),
            )
    ...
    def test_non_binary_y_train(self, valid_X_test, valid_y_test, valid_X_train):
        with pytest.raises(ValueError):
            DataSet(
                ...
                y_test=valid_y_test,
                X_train=valid_X_train,
                y_train=np.array([0, 0, 1, 0.5, 1, 0, 0]),
            )

    def test_train_data_and_labels_different_shape(
        self, valid_X_test, valid_y_test, valid_X_train, valid_y_train
    ):
        with pytest.raises(ValueError):
            DataSet(
                ...
                y_test=valid_y_test,
                X_train=valid_X_train,
                y_train=np.append(valid_y_train, 0),
            )
...
class TestTimeSteps:
    ...
    def test_valid_time_steps_test(self, valid_X_test, valid_y_test):
        ...
        data_set = DataSet(
            X_test=valid_X_test, y_test=valid_y_test, time_steps_test=time_steps
        )
    ...
    def test_time_steps_test_different_size(self, valid_X_test, valid_y_test):
        with pytest.raises(ValueError):
            DataSet(
                X_test=valid_X_test,
                y_test=valid_y_test,
                time_steps_test=np.arange(0, valid_X_test.shape[0] + 1),
            )
    ...
    def test_time_steps_train_different_size(
        self, valid_X_test, valid_y_test, valid_X_train
    ):
        with pytest.raises(ValueError):
            DataSet(
                ...
                y_test=valid_y_test,
                X_train=valid_X_train,
                time_steps_train=np.arange(0, valid_X_train.shape[0] + 1),
            )
    ...
    def test_time_steps_train_no_x_train(self, valid_X_test, valid_y_test):
        with pytest.raises(ValueError):
            DataSet(
                X_test=valid_X_test,
                y_test=valid_y_test,
                time_steps_train=np.arange(0, valid_X_test.shape[0]),
            )


class TestCompatibleSupervision:

    def test_unsupervised_all_data(
        self, valid_X_test, valid_y_test, valid_X_train, valid_y_train
    ):
        data_set = DataSet(
            X_test=valid_X_test,
    ...
    def test_unsupervised_no_train_labels(
        self, valid_X_test, valid_y_test, valid_X_train
    ):
        data_set = DataSet(
            X_test=valid_X_test, y_test=valid_y_test, X_train=valid_X_train
    ...
    def test_semi_supervised_all_data(
        self, valid_X_test, valid_y_test, valid_X_train, valid_y_train
    ):
        data_set = DataSet(
            X_test=valid_X_test,
    ...
    def test_semi_supervised_no_train_labels(
        self, valid_X_test, valid_y_test, valid_X_train
    ):
        data_set = DataSet(
            X_test=valid_X_test, y_test=valid_y_test, X_train=valid_X_train
    ...
    def test_supervised_no_train_labels(
        self, valid_X_test, valid_y_test, valid_X_train
    ):
        data_set = DataSet(
            X_test=valid_X_test, y_test=valid_y_test, X_train=valid_X_train

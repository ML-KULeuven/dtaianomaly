...
class TestCustomDataLoader:
    ...
    def test_semi_supervised_zero_labels(self, time_train, time_test, tmp_path):
        write_semi_supervised_data_zero_labels(
            tmp_path / "semi_supervised.csv", tmp_path / "semi_supervised_train.csv"
        )
        if not time_train:
            drop_time(tmp_path / "semi_supervised_train.csv")
    ...
    def test_different_nb_features(self, tmp_path):
        ...
        pd.read_csv(tmp_path / "supervised.csv").drop(columns="f1").to_csv(
            tmp_path / "supervised.csv", index=False
        )
    ...
    def test_different_feature_names(self, tmp_path):
        ...
        pd.read_csv(tmp_path / "supervised.csv").rename(
            columns={"f1": "other-name"}
        ).to_csv(tmp_path / "supervised.csv", index=False)
        loader = CustomDataLoader(
            tmp_path / "supervised.csv", tmp_path / "supervised_train.csv"
        )
        with pytest.raises(ValueError):
    ...
    def test_shuffled_columns(self, tmp_path):
        ...
        pd.read_csv(tmp_path / "supervised.csv")[
            ["time", "label", "f2", "f3", "f1"]
        ].to_csv(tmp_path / "supervised.csv", index=False)
        loader = CustomDataLoader(
            tmp_path / "supervised.csv", tmp_path / "supervised_train.csv"

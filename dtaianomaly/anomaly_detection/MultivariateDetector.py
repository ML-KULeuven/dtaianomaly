...
class MultivariateDetector(BaseDetector):
    ...
    def __init__(
        ...
        if not isinstance(detector, BaseDetector):
            raise TypeError(
                "`detector` must be a dtaianomaly.anomaly_detection.BaseDetector"
            )
        ...
        if aggregation not in ["min", "max", "mean"]:
            raise ValueError("`aggregation` should be one of {'min', 'max', 'mean'}")
        if not isinstance(raise_warning_for_univariate, bool):
            raise TypeError("'raise_warning_for_univariate' should be a boolean")
    ...
    def _decision_function(self, X: np.ndarray) -> np.array:
        ...
        if self.aggregation == "min":
            ...
            raise ValueError(
                f"Invalid value found for aggregation: '{self.aggregation}'!"
            )
